...
export function LockBracketButton({ userId }: { userId: string }) {
  ...
  function handle() {
    if (!confirm('Vas a CONFIRMAR el bracket de este usuario en su nombre. Sus picks de eliminatorias (campeón, sub, 3°, 4°) y su goleador empezarán a contar en el ranking y quedará bloqueado. ¿Seguro?')) return;
    start(async () => {
  ...
  return (
    ...
    >
      {pending ? 'Confirmando…' : '🔒 Confirmar bracket de este usuario'}
    </button>
  );
}
